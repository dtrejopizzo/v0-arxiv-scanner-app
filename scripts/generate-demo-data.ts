 ...
 * This fetches real papers from arXiv and generates synthetic analyses
 * to populate the app with demo data for popular categories.
 *
 * In production, the admin page calls Gemini for real AI analysis.
 * This script is just for demo/testing purposes.
 */
...
function generateSyntheticAnalysis(paper: ArxivPaper, index: number): PaperAnalysis {
  ...
  const claimTemplates = [
    `Proposes a novel approach to ${paper.title.split(" ").slice(0, 4).join(" ").toLowerCase()}`,
  ...
  const commentaryTemplates = [
    `This is a solid piece of work that tackles ${paper.categories[0] || "the field"} from an interesting angle. The methodology is sound, though the evaluation could be more comprehensive. Worth reading if you're in this space.`,
  ...
  const oneLinerTemplates = [
    `New method for ${paper.categories[0] || "the problem"} that shows promising results on standard benchmarks.`,
...
async function generateForCategory(category: { code: string; name: string }) {
  ...
  const cachedData = {
    ...
    analyzedCount: analyzedPapers.filter((p) => p.analysis).length,
    papers: analyzedPapers,
    sotaRanking,
  }

  // Write to public/data/analyses/
  ...
  console.log(`  Wrote ${filePath} (${papers.length} papers, ${sotaRanking.length} SOTA)`)
}
